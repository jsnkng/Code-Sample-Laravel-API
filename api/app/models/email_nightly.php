...
class EmailNightly
{
    ...
    *******************************************************************************/
    public function __construct($form, $return_id, $mgKey, $mgDomain, $email_to)
    {
        ...
        $this->_view_log->pushHandler(new StreamHandler(dirname(dirname(__DIR__)).'/app/storage/logs/email_nightly.log', Logger::INFO));

        //Email template
        $api_dir = realpath('../');
        $email_dir = $api_dir."/app/views/emails/nightly/";
        $email_file = "syna14cdpr0345_transaction_summary.html";
